...
xmobarSB :: StatusBarConfig
xmobarSB   = statusBarPropTo "_XMONAD_LOG_0" (xmobar 0) (pure myXmobarPP')
...
xmobar i =
  ...
  [ "xmobar",
      "-B", translate black,
      "-F", translate white,
      "-f", "xft:IosevkaNerdFont:pixelsize=12:antialias=true:hinting=true",
      "-N", "xft:FontAwesome:size=11",
      "-i", xmonadConfigHome <> "/xpm",
      "-x", show i,
      "-p", translate "TopH 24",
      "-t", translate (xmobarTemplate),
      "-c", translate $ list (xmobarCommands)
    ]


xmobarTemplate :: String
xmobarTemplate = concat $
...
xmobarCommands = map unwords $
  [xPropLog,tiramisu, online,date,locks,battery,tray,cpu,memory,thermal] --,locks,battery,volume,thermal,cpu,memory,tray]
  where
    xPropLog = [ "Run  UnsafeXPropertyLog \"_XMONAD_LOG_0\""  ]
    tiramisu     = ["Run CommandReader", quote (xmonadConfigHome <> "/scripts/tiramisu.sh"), quote "tiramisu"]
...
main =  do
    ...
    xmonad . ewmh . docks . withSB xmobarSB $ def -- dynamicSBs barSpawner' $ def
...
myStartupHook = do
    ...
    setWMName "LG3D"
    spawnOnce "redshift -xPO 2500"
    spawnOnce "picom -b --backend glx --glx-fshader-win \"uniform sampler2D tex; uniform float opacity; void main() { vec4 c = texture2D(tex, gl_TexCoord[0].xy); float y = dot(c.rgb, vec3(0.2126, 0.7152, 0.0722)); gl_FragColor = opacity*vec4(y, y, y, c.a); }\""
...
keybinds conf@XConfig { XMonad.modMask = modm } =
  M.fromList
    ...
    --useful stuff to have binded
         ...
         --((modm, xK_Return), safeSpawn "emacsclient" ["-c"]),
         --spawn a terminal
    ...
    -- Kill windows
         ...
         ((altMask, xK_space), safeSpawn "xdotool" ["click", "2"]),
         ((altMask .|. controlMask, xK_i), safeSpawn "rofi" ["-modi","emoji","-show", "emoji"]),
