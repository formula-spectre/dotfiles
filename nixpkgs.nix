    ...
    in
{
 ...
 if [  "$(tty)" = "/dev/tty1" ] && [ -z $DISPLAY ]; then
   _s6_compdir=$XDG_CONFIG_HOME/s6u/rc/compiled
   _s6_dir=$XDG_CONFIG_HOME/s6u
   _s6_scandir=/tmp/$USER/service
   _s6_livedir=/tmp/$USER/s6-rc
   _s6_logger_fifo="$s6_scandir/s6-svscan-log/fifo"

   mkdir -p "$_s6_scandir"
   mkdir -p "$_s6_dir/log"
   cp -rL "$_s6_dir/s6-svscan-log $s6_scandir"
   mkfifo "$_s6_logger_fifo"
   setsid -f \
       fdmove -c 3 2 \
           redirfd -wnb 1 "_s6_logger_fifo" \
           fdmove -c 2 1  \
               s6-envdir "$_s6_dir/envdir" \
               s6-svscan "$_s6_scandir"
